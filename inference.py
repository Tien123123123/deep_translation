...
from arg_parser.arg_inference import arg_inference
...
def inference(args):
    ...
    yolo_model, ocr_model = set_up_models(args)
    set_up_inference()

    print("Text detection processing...")
    images_dict = inference_detection(yolo_model, args)
    ...
    for key, val in data_dict.items():
        ...
        if " " not in label or str(label).isalpha():
            label = inference_translate(label, args)
        else:
            label = seperate_word(label)
            label = flatten_and_convert(label, args)
        data_dict[key] = {
            'bnd_box': val['bnd_box'],
            'label_recognition': label
        }

    put_text(data_dict, args)
